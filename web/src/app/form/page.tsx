...
export default function FormPage() {
    ...
    useEffect(() => {
        const generateSampleQR = async () => {
            try {
                // Increased width for better print resolution
                const url = await QRCode.toDataURL('CGI-SAMPLE-QR', { margin: 1, width: 250 });
                setQrCodeUrl(url);
            } catch (err) {
                console.error("Failed to generate sample QR:", err);
            }
        };
        generateSampleQR();
    }, []);
    ...
    <tr>
      ...
      <td rowspan="3" colspan="3" class="input">
        <div class="qr-container">
            ${qrCodeUrl ? `<img src="${qrCodeUrl}" alt="QR Code" />` : '<div style="font-size:8px; color:#ccc;">GENERATING QR...</div>'}
        </div>
      </td>
            ...
            <Card className="w-full max-w-5xl mx-auto">
                <CardHeader>
                    <div className="flex justify-between items-start">
                        ...
                        <Button onClick={handlePrint} variant="outline" disabled={!qrCodeUrl}>
                            {!qrCodeUrl && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
