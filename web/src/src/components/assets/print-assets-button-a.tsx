...
export default function PrintAssetsButtonA({ selectedAssets }: PrintAssetsButtonAProps) {
  ...
  const handlePrint = () => {
    ...
    const uniqueLocations = Array.from(new Set(selectedAssets.map(asset => asset.location))).join(', ');
    ...
    for (let i = selectedAssets.length; i < 40; i++) {
        tableRows += `
            <tr>
                <td>${i + 1}</td>
                <td></td>
