...
# 1. Start from restored_page.txt
...
if start_idx != -1 and end_idx != -1:
    ...
    print("Could not find start/end markers in restored_page.txt")
    exit(1)
...
# 4. Insert components at the top, right before `export default function RegisterDesignPage() {`
helpers = """
...
# 5. Remove inner CellInput and CellSelect cleanly using exact boundaries
start_rem = content.find("  // Helper for input\n  const CellInput =")
if start_rem != -1:
    end_rem = content.find("  const filteredData = data.filter(", start_rem)
    if end_rem != -1:
        content = content[:start_rem] + content[end_rem:]
    else:
        print("Could not find filteredData!")
else:
    print("Could not find start_rem!")


# 6. Change `useAuthState(auth)` to `useAuth()`
content = content.replace("const [user, loadingUser] = useAuthState(auth);", "const { user, loading: loadingUser } = useAuth();")


# 7. Inject updated states inside RegisterDesignPage()
...
old_state_start = content.find("  const [typeDesignOptions,")
old_state_end = content.find("  // Grouping\n  const [selectedIds,")
if old_state_start != -1 and old_state_end != -1:
    content = content[:old_state_start] + state_decl + "\n" + content[old_state_end:]
else:
    print("Could not replace old_state!")


# 8. Update fetchData logic
...
# 10. Update Rows - Using exact slice replacement to avoid regex dotall bugs!
start_row = content.find('<td className="p-1 border-r"><CellInput row={row} field="customer" width="w-32" /></td>')
end_row = content.find('<td className="p-1 border-r"><CellInput row={row} field="requiredDate" width="w-24" /></td>', start_row)
if start_row != -1 and end_row != -1:
    end_row += len('<td className="p-1 border-r"><CellInput row={row} field="requiredDate" width="w-24" /></td>')
    ...
    content = content[:start_row] + row_new + content[end_row:]
else:
    print("Could not find rows to replace!")


# 11. Replace colspans and inject handleUpdateCell into remaining CellInputs
...
# 12. Update Datalists
start_dl = content.find('<datalist id="desOptions">')
end_dl = content.find('      <Dialog open={isGroupDialogOpen}', start_dl)
if start_dl != -1 and end_dl != -1:
    dl_new = """<datalist id="desOptions">{designerOptions.map(o => <option key={o} value={o} />)}</datalist>
...
"""
    content = content[:start_dl] + dl_new + content[end_dl:]
else:
    print("Could not find datalists to replace!")
